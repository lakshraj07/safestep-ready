...
- **Primary surfaces:** iPhone walkthrough → generated clinician report in the browser.
...
Relay conducts the post-encounter follow-through. Monica’s caregiver walks the home with an iPhone. Relay reads the chart, guides the walkthrough, sees hazards, measures the environment, asks clarifying questions, checks whether the care plan is feasible, and creates the appropriate actions for the care team.
...
> “Relay turns the care plan into a real-world follow-through workflow. Monica’s niece can walk the apartment with an iPhone before Monica comes home.”

Do not explain LiDAR, Haiku, Sonnet, Vapi, FastAPI, or FHIR here.
...
Medplum holds the chart. SafeStep carries the plan into the home — because care does not end when the encounter does.
...
- Pre-capture the difficult bathroom and doorway portion using the real iPhone and RoomPlan.
...
- Mac IP and iPhone backend host match.
- ngrok/custom-LLM endpoint is current.
- Vapi voice connects from the venue network.
- iPhone is charged and connected to backup power.
...
- Notifications and sleep are disabled on Mac and iPhone.
