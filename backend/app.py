"""FastAPI backend: dumb iPhone in, smart clinical assessment out.
...
Endpoints:
  ...
  POST /v1/chat/completions   OpenAI-compatible SSE for Vapi custom LLM
...
@app.get("/runs")
def runs_json():
    """Machine-readable run list for the iPhone's past-walkthrough browser."""
...
def index():
    ...
    live = state.current()
    return f"""<!doctype html><html><head><meta charset="utf-8"><title>SafeStep — runs</title>
...
<h1>SafeStep · walkthrough runs</h1>
